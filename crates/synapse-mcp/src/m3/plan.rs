...
mod tests {
    ...
    #[test]
    fn app_only_steps_compile_to_launch_with_window_postcondition() {
        let s = compile_step(0, &step("code.exe", None));
        assert_eq!(s.backend, PlanBackend::ActLaunch);
        assert!(s.deterministic);
        assert_eq!(
            s.postcondition,
            Postcondition::WindowForProcessExists {
                process: "code.exe".to_owned()
            }
        );
    }

    #[test]
    fn browser_with_host_compiles_to_background_tab() {
        let s = compile_step(1, &step("chrome.exe", Some("mail.google.com")));
        assert_eq!(s.backend, PlanBackend::CdpOpenTab);
        assert!(s.deterministic);
        assert_eq!(
            s.postcondition,
            Postcondition::BrowserTabAtHost {
                host: "mail.google.com".to_owned()
            }
        );
        assert!(s.action.contains("https://mail.google.com"));
    }

    #[test]
    fn browser_with_page_title_degrades_to_agent_task() {
        let s = compile_step(0, &step("chrome.exe", Some("Inbox (42) - Gmail")));
        assert_eq!(s.backend, PlanBackend::AgentTask);
        assert!(!s.deterministic);
        assert!(s.agent_task_reason.is_some());
    }

    #[test]
    fn non_browser_with_document_degrades_to_agent_task_never_dropped() {
        let s = compile_step(0, &step("excel.exe", Some("Q3 report")));
        assert_eq!(s.backend, PlanBackend::AgentTask);
        assert!(!s.deterministic);
        assert_eq!(
            s.postcondition,
            Postcondition::DocumentWindowOpen {
                app: "excel.exe".to_owned(),
                document: "Q3 report".to_owned()
            }
        );
    }

    #[test]
    fn mixed_routine_counts_deterministic_and_agent_steps() {
        let routine = RoutineRecord {
            ...
            steps: vec![
                step("chrome.exe", Some("mail.google.com")), // deterministic tab
                step("code.exe", None),                      // deterministic launch
                step("excel.exe", Some("Q3 report")),        // agent task
            ],
            dow_class: synapse_core::types::RoutineDowClass::Daily,
            ...
            window_start_ns: 0,
            window_end_ns: 0,
            active_days_in_window: 1,
            first_seen_day_start_ns: 0,
            last_seen_day_start_ns: 0,
            evidence: vec![],
        };
        let plan = compile_plan(&routine, 123);
        assert_eq!(plan.total_steps, 3);
        assert_eq!(plan.deterministic_steps, 2);
        assert_eq!(plan.agent_task_steps, 1);
        assert!(!plan.fully_deterministic);
        // No step is ever dropped.
