...
mod tests {
    ...
    #[test]
    fn browser_dnd_validates_defaults_for_drag_and_drop() {
        let params = BrowserDndParams {
            source_selector: "#source".to_owned(),
            target_selector: "#target".to_owned(),
            mode: None,
            steps: None,
            duration_ms: None,
            data_mime_type: None,
            ...
            auto_wait: None,
            auto_wait_timeout_ms: None,
        };
        let drag = validate_browser_dnd_params(&params, BrowserDndMode::Mouse).unwrap();
        assert_eq!(drag.mode, BrowserDndMode::Mouse);
        assert_eq!(drag.steps, DEFAULT_MOUSE_STEPS);
        assert_eq!(drag.duration_ms, DEFAULT_MOUSE_DURATION_MS);

        let drop = validate_browser_dnd_params(&params, BrowserDndMode::Html5).unwrap();
        assert_eq!(drop.mode, BrowserDndMode::Html5);
    }

    #[test]
    fn browser_dnd_rejects_bad_edges() {
        let mut params = BrowserDndParams {
            source_selector: " ".to_owned(),
            target_selector: "#target".to_owned(),
            mode: None,
            steps: None,
            duration_ms: None,
        ...
        assert!(validate_browser_dnd_params(&params, BrowserDndMode::Mouse).is_err());

        params.source_selector = "#source".to_owned();
        params.steps = Some(0);
        assert!(validate_browser_dnd_params(&params, BrowserDndMode::Mouse).is_err());

        params.steps = Some(1);
        params.duration_ms = Some(10_001);
        assert!(validate_browser_dnd_params(&params, BrowserDndMode::Mouse).is_err());
    }
}
