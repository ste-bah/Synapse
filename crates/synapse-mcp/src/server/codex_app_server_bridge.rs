...
mod tests {
    ...
    fn envelope(method: &str, params: Value) -> CodexAppServerRequestEnvelope {
        CodexAppServerRequestEnvelope {
            spawn_id: "agent-spawn-codex-bridge-test".to_owned(),
    ...
    #[test]
    fn command_approval_maps_to_agent_permission_without_edit_affordance() {
        let spec = request_spec(&envelope(
            "item/commandExecution/requestApproval",
            json!({
                "threadId": "thread-a",
        ...
        assert_eq!(
            spec.request.dedupe_key.as_deref(),
            Some(
                "codex:agent-spawn-codex-bridge-test:item/commandExecution/requestApproval:item-a:default"
            )
        );
        assert!(spec.request.body.contains("Remove-Item"));
    }

    #[test]
    fn request_user_input_maps_operator_response_to_first_question() {
        let spec = request_spec(&envelope(
            "item/tool/requestUserInput",
            json!({
                "threadId": "thread-a",
                ...
                "itemId": "item-q",
                "questions": [
                    { "id": "answer_one", "question": "Continue?", "header": "Need input", "isOther": false, "isSecret": false, "options": null }
                ],
                "autoResolutionMs": null
            }),
        ))
        .expect("spec");
        assert_eq!(spec.kind, ApprovalKind::AgentQuestion);
        let mut item = ApprovalItemRecord {
            schema_version: 1,
            approval_id: "apr1-test".to_owned(),
            ...
            timeout_decision: ApprovalTimeoutDecision::Declined,
            requested_by_session: "test".to_owned(),
            decided_by_session: Some("operator".to_owned()),
            decided_at_unix_ms: Some(2),
            decision_note: None,
            allow: ApprovalAllow::for_kind(ApprovalKind::AgentQuestion),
            edited_args_json: None,
            operator_response: Some("yes".to_owned()),
            toast: crate::m3::approvals::ApprovalToastState {
                requested: false,
                suppress_popup: false,
        ...
        assert_eq!(response["answers"]["answer_one"]["answers"][0], "yes");
        item.status = ApprovalStatus::Declined;
        let declined = app_server_response_for(&spec, &item);
        assert_eq!(declined["answers"], json!({}));
    }

    #[test]
    fn mcp_tool_elicitation_maps_to_agent_permission() {
        let spec = request_spec(&envelope(
            "mcpServer/elicitation/request",
            json!({
                "threadId": "thread-a",
                "turnId": "turn-a",
                "serverName": "synapse",
                "message": "Allow the synapse MCP server to run tool \"health\"?",
                "mode": "form",
                "requestedSchema": { "type": "object", "properties": {} },
                "_meta": {
                    "codex_approval_kind": "mcp_tool_call",
                    "tool_description": "Return server health",
                    "tool_params": {},
                    "tool_params_display": []
                }
            }),
        ...
        let item = ApprovalItemRecord {
            ...
            decided_at_unix_ms: Some(2),
            decision_note: None,
            allow: ApprovalAllow::for_kind(ApprovalKind::AgentPermission),
            edited_args_json: None,
