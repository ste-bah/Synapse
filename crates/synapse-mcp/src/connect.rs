...
fn capture_process_creation_time_100ns(parent_pid: u32) -> anyhow::Result<u64> {
    let identity = capture_process_identity(parent_pid)?;
    let creation_time_100ns = identity.creation_time_100ns;
    identity.close_checked().with_context(|| {
        format!("close exact identity-capture handle for parent pid {parent_pid}")
    })?;
...
mod tests {
    ...
    #[tokio::test]
    async fn parent_watchdog_channel_preserves_exit_and_channel_failure() {
        let (sender, receiver) = oneshot::channel();
        sender
            .send(ParentWatchdogEvent::ParentExited {
                parent_pid: 42,
                parent_creation_time_100ns: 7,
            })
            .expect("watchdog receiver should be live");
        assert!(matches!(
            wait_for_parent_watchdog(Some(receiver)).await,
            ParentWatchdogEvent::ParentExited {
                parent_pid: 42,
                parent_creation_time_100ns: 7,
            }
        ...
        let (sender, receiver) = oneshot::channel::<ParentWatchdogEvent>();
        drop(sender);
        assert!(matches!(
            wait_for_parent_watchdog(Some(receiver)).await,
            ParentWatchdogEvent::Failed {
    ...
    #[tokio::test]
    async fn transport_close_aggregation_retains_error_and_timeout() {
        let mut failures = ConnectShutdownFailures::default();
        failures.inspect_close(
            ConnectTransport::Daemon,
            Duration::from_millis(50),
            Ok::<Result<(), &'static str>, tokio::time::error::Elapsed>(Err(
                "daemon close channel failed",
            )),
        );
        let client_timeout = tokio::time::timeout(Duration::from_millis(1), async {
        ...
        assert_eq!(failures.failures[0].code, "MCP_CONNECT_DAEMON_CLOSE_FAILED");
        assert_eq!(
            failures.failures[1].code,
            "MCP_CONNECT_CLIENT_CLOSE_TIMEOUT"
        );
        let error = failures
            .into_result()
            .expect_err("either close failure must prevent a successful bridge verdict");
        let detail = error.to_string();
        assert!(detail.contains("MCP_CONNECT_DAEMON_CLOSE_FAILED"));
        assert!(detail.contains("MCP_CONNECT_CLIENT_CLOSE_TIMEOUT"));
    }

    #[test]
    fn session_cleanup_requires_a_finished_delete_for_every_observed_session() {
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session("session-a");
        let active = cleanup
            .verification_error()
            .expect("an active session cannot be accepted as deleted");
        assert!(active.contains("session_id=session-a state=active"));

        let session_a_delete = cleanup.begin_delete("session-a");
        let started = cleanup
            .verification_error()
            .expect("an in-flight DELETE cannot be accepted as complete");
        assert!(started.contains("state=delete_started"));

        cleanup.finish_delete(&session_a_delete, &Ok(ConnectSessionDeleteSuccess::Deleted));
        assert!(cleanup.verification_error().is_none());

        cleanup.observe_session("session-expired");
        let expired_delete = cleanup.begin_delete("session-expired");
        cleanup.finish_delete(&expired_delete, &Ok(ConnectSessionDeleteSuccess::Absent));
        assert!(
            cleanup.verification_error().is_none(),
            "an explicit daemon absence readback is terminal cleanup"
        );
    }

    #[test]
    fn session_delete_failure_is_sticky_across_a_later_success() {
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session("session-reused");
        let first_delete = cleanup.begin_delete("session-reused");
        let failed: Result<ConnectSessionDeleteSuccess, StreamableHttpError<reqwest::Error>> =
            Err(StreamableHttpError::ServerDoesNotSupportDeleteSession);
        cleanup.finish_delete(&first_delete, &failed);

        cleanup.observe_session("session-reused");
        let second_delete = cleanup.begin_delete("session-reused");
        cleanup.finish_delete(&second_delete, &Ok(ConnectSessionDeleteSuccess::Deleted));

        let detail = cleanup
            .verification_error()
            .expect("later success must not erase immutable failure evidence");
        assert!(detail.contains("immutable_delete_failures"));
        assert!(detail.contains("Server does not support delete session"));
    }

    #[test]
    fn reused_session_id_keeps_distinct_cleanup_generations() {
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session("session-reused-after-restart");
        let old_generation = cleanup.begin_delete("session-reused-after-restart");
        cleanup.finish_delete(&old_generation, &Ok(ConnectSessionDeleteSuccess::Absent));

        cleanup.observe_session("session-reused-after-restart");
        let new_generation = cleanup.begin_delete("session-reused-after-restart");
        cleanup.finish_delete(&new_generation, &Ok(ConnectSessionDeleteSuccess::Deleted));

        assert_ne!(
            old_generation.observation_id, new_generation.observation_id,
            "daemon restart/reinitialization must create a new ledger generation even if the opaque ID is reused"
        );
        let observation_count = cleanup.with_ledger(|ledger| ledger.sessions.len());
        assert_eq!(observation_count, 2);
        assert!(cleanup.verification_error().is_none());
    }

    #[test]
    fn reused_id_cannot_collapse_an_unresolved_older_generation() {
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session("same-opaque-id");
        cleanup.observe_session("same-opaque-id");

        let newest_delete = cleanup.begin_delete("same-opaque-id");
        cleanup.finish_delete(&newest_delete, &Ok(ConnectSessionDeleteSuccess::Deleted));

        let detail = cleanup
            .verification_error()
            .expect("the older generation must remain visibly unresolved");
        assert!(detail.contains("observation_id=0"), "{detail}");
        assert!(detail.contains("state=active"), "{detail}");
    }

    #[test]
    fn rmcp_redundant_delete_preserves_prior_terminal_absence() {
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session("expired-before-failed-reinit");
        let absence_readback = cleanup.begin_delete("expired-before-failed-reinit");
        cleanup.finish_delete(&absence_readback, &Ok(ConnectSessionDeleteSuccess::Absent));

        let rmcp_finalizer = cleanup.begin_delete("expired-before-failed-reinit");
        assert!(rmcp_finalizer.terminal_before_attempt);
        let redundant_transport_failure: Result<
            ConnectSessionDeleteSuccess,
            StreamableHttpError<reqwest::Error>,
        > = Err(StreamableHttpError::ServerDoesNotSupportDeleteSession);
        cleanup.finish_delete(&rmcp_finalizer, &redundant_transport_failure);

        assert!(
            cleanup.verification_error().is_none(),
            "a redundant rmcp finalizer cannot erase an earlier physical absence verdict"
        );
    }

    #[test]
    fn only_structured_synapse_404_is_terminal_absence_evidence() {
        let session_id = "session-expired";
        ...
        verify_explicit_session_absence(
            ...
            Some("reinitialize"),
            explicit.to_string().as_bytes(),
        )
        .expect("the exact session middleware payload proves physical absence");

        let generic = br#"{"code":"NOT_FOUND","session_id":"session-expired"}"#;
        let error = verify_explicit_session_absence(
            session_id,
            Some("application/json"),
            Some("reinitialize"),
            generic,
        )
        .expect_err("a generic route/proxy 404 must not become false cleanup success");
        assert!(
            error.contains("expected code=HTTP_SESSION_INVALID"),
            "{error}"
        );
    }

    #[tokio::test]
    async fn expired_post_obtains_terminal_absence_before_rmcp_reinitialization()
    -> anyhow::Result<()> {
        use tokio::io::{AsyncReadExt, AsyncWriteExt};

        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await?;
        let address = listener.local_addr()?;
        let session_id = "expired-before-reinit";
        let absence_body = serde_json::json!({
            "code": synapse_core::error_codes::HTTP_SESSION_INVALID,
            "reason": "unknown_or_expired",
            "failure_class": "session_invalid",
            "daemon_alive": true,
            ...
            for response_index in 0..2 {
                let (mut stream, _peer) =
                    tokio::time::timeout(Duration::from_secs(2), listener.accept())
                        .await
                        .context("wait for tracked POST/DELETE request")??;
                let mut request = Vec::new();
                let header_end = loop {
                    let mut chunk = [0_u8; 1024];
                    let read = stream.read(&mut chunk).await?;
                    anyhow::ensure!(read > 0, "request ended before its HTTP headers");
                ...
                while request.len() - header_end < content_length {
                    let mut chunk = [0_u8; 1024];
                    let read = stream.read(&mut chunk).await?;
                    anyhow::ensure!(read > 0, "request ended before its HTTP body");
                    request.extend_from_slice(&chunk[..read]);
                }
                let method = headers
                    .lines()
                ...
                let response = if response_index == 0 {
                    ...
                    format!(
                        "HTTP/1.1 404 Not Found\r\nConnection: close\r\nContent-Type: application/json; charset=utf-8\r\nMcp-Session-Recovery: reinitialize\r\nContent-Length: {}\r\n\r\n{}",
                        absence_body.len(),
                        absence_body
                    )
                };
        ...
        let cleanup = ConnectSessionCleanupTracker::default();
        cleanup.observe_session(session_id);
        let (owner, registration) = ConnectHttpOwner::new();
        let client = TrackedConnectHttpClient {
        ...
        .await?;
        assert!(
            matches!(result, Err(StreamableHttpError::SessionExpired)),
            "rmcp must still receive its reinitialization control signal"
        );
        assert!(
            cleanup.verification_error().is_none(),
            "the old session generation must have explicit terminal absence before reinit"
        );
        let methods = server.await??;
        assert_eq!(methods.len(), 2);
        assert_eq!(methods[0], "POST");
        assert_eq!(methods[1], "DELETE");
        drop(client);
        owner.wait_for_quiescence().await?;
        Ok(())
    }

    #[test]
    fn stdio_receive_end_is_clean_only_after_zero_byte_eof_read() {
        assert_eq!(
            client_receive_end_reason(true).expect("observed EOF is a normal close"),
            "stdin_eof"
        );
        let error = client_receive_end_reason(false)
            .expect_err("an unclean stdio receive failure must make the bridge nonzero")
            .to_string();
        assert!(
            error.contains("MCP_CONNECT_CLIENT_TRANSPORT_CLOSED_WITHOUT_EOF"),
            "{error}"
        );
    }

    #[tokio::test]
    async fn exact_http_client_owner_waits_for_every_clone_to_drop() {
        let cleanup = ConnectSessionCleanupTracker::default();
        let (owner, registration) = ConnectHttpOwner::new();
        let client = TrackedConnectHttpClient {
        ...
        assert_eq!(owner.live_clients(), 2);
        drop(client);
        assert_eq!(owner.live_clients(), 1);
        drop(clone);
        owner
            .wait_for_quiescence()
            .await
            .expect("every exact wrapper owner reached terminal Drop");
        assert_eq!(owner.live_clients(), 0);
    }

    #[test]
    fn exact_parent_exit_is_an_abnormal_bridge_verdict() {
        let mut failures = ConnectShutdownFailures::default();
        failures.push(
            "MCP_CONNECT_PARENT_EXITED",
            "parent_watchdog",
            "parent_pid=42 parent_creation_time_100ns=99 phase=bridge_active",
        );
        let detail = failures
            .into_result()
            .expect_err("parent death must not masquerade as verified stdin EOF")
            .to_string();
        assert!(detail.contains("MCP_CONNECT_PARENT_EXITED"), "{detail}");
    }

    #[test]
    fn both_transport_close_successes_are_clean() {
        let mut failures = ConnectShutdownFailures::default();
        failures.inspect_close(
            ConnectTransport::Daemon,
            DAEMON_CLOSE_TIMEOUT,
            Ok::<Result<(), &'static str>, tokio::time::error::Elapsed>(Ok(())),
        );
        failures.inspect_close(
            ConnectTransport::Client,
            CLIENT_CLOSE_TIMEOUT,
        ...
        failures
            .into_result()
            .expect("both verified close results should permit success");
    }

    #[cfg(windows)]
    #[test]
    fn process_creation_identity_is_stable_across_real_handle_reads() -> anyhow::Result<()> {
        use windows::Win32::{
        ...
        let first = capture_process_creation_time_100ns(pid)?;
        // SAFETY: this test opens its own process with query-only access and
        // closes the returned handle exactly once after the independent read.
        let handle = unsafe { OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, pid) }?;
        let second = process_creation_time_100ns(handle);
        let close = unsafe { CloseHandle(handle) };
        let second = second?;
        close?;

        assert_eq!(first, second);
        assert!(creation_time_unix_secs(first).is_some());
        Ok(())
    }

    #[cfg(windows)]
    #[test]
    fn subsecond_parent_creation_difference_is_not_collapsed_to_sysinfo_seconds() {
        let first = WINDOWS_TO_UNIX_EPOCH_100NS + 123 * 10_000_000 + 1;
        let replacement = first + 1;

        assert_eq!(
            creation_time_unix_secs(first),
            creation_time_unix_secs(replacement),
            "the synthetic processes intentionally occupy the same sysinfo second"
        );
        assert!(!exact_parent_creation_matches(first, replacement));
        assert!(exact_parent_creation_matches(first, first));
    }

    #[cfg(windows)]
    #[test]
    fn parent_snapshot_binds_pid_to_exact_live_creation_time() -> anyhow::Result<()> {
        let mut parent = parent_process_info()?
            .ok_or_else(|| anyhow::anyhow!("test process has no observable parent"))?;
        let identity = parent
            .identity
            .take()
            .ok_or_else(|| anyhow::anyhow!("parent snapshot did not retain its exact handle"))?;
        let retained_handle_read = process_creation_time_100ns(identity.handle()?)?;
        let kernel_snapshot = kernel_parent_process_snapshot()?
            .ok_or_else(|| anyhow::anyhow!("test process has no exact kernel parent snapshot"))?;

        assert_eq!(identity.pid, parent.pid);
        assert_eq!(kernel_snapshot.pid, parent.pid);
        assert_eq!(kernel_snapshot.creation_time_100ns, retained_handle_read);
        assert_eq!(parent.creation_time_100ns, Some(retained_handle_read));
        assert_eq!(identity.creation_time_100ns, retained_handle_read);
        assert_eq!(
            creation_time_unix_secs(retained_handle_read),
            Some(parent.start_time_unix_secs)
        );
        identity.close_checked()?;
        Ok(())
    }

    #[cfg(windows)]
    #[test]
    fn direct_wsl_interop_parent_signatures_fail_closed() {
        let parent = |name: &str, command_line: &str| ParentProcessInfo {
            pid: 42,
            name: name.to_owned(),
        ...
        assert!(parent("wsl.exe", "wsl.exe -d Ubuntu").is_unsupported_wsl_interop_host());
        assert!(parent("wslhost.exe", "wslhost.exe --vm-id 1").is_unsupported_wsl_interop_host());
        assert!(
            parent(
                "conhost.exe",
                r"conhost.exe --headless C:\Windows\System32\wsl.exe -d Ubuntu"
            )
            .is_unsupported_wsl_interop_host()
        );
        assert!(
            !parent("claude.exe", "claude.exe --mcp-config config.json")
                .is_unsupported_wsl_interop_host()
        );
    }
}
