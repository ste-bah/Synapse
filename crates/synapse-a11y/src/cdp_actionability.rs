...
mod tests {
    ...
    fn dom_state() -> CdpActionabilityDomState {
        CdpActionabilityDomState {
            attached: true,
            tag_name: "button".to_owned(),
            id: "save".to_owned(),
            class_name: String::new(),
            name_attr: String::new(),
            type_attr: "text".to_owned(),
            display: "inline-block".to_owned(),
            visibility: "visible".to_owned(),
            pointer_events: "auto".to_owned(),
            disabled: false,
            aria_disabled: false,
            readonly: false,
            enabled: true,
            editable: false,
            viewport_rect: rect(10.0, 20.0, 40.0, 20.0),
            node_description: "button#save".to_owned(),
        }
    }
    ...
    fn rect(x: f64, y: f64, width: f64, height: f64) -> CdpActionabilityRect {
        CdpActionabilityRect {
            x,
            y,
            width,
            height,
    ...
    #[test]
    fn visible_requires_box_area_and_css_visibility() {
        let mut state = dom_state();
        let box_model = model(10.0, 20.0, 40.0, 20.0);
        assert!(visible_from(&state, Some(&box_model)));

        state.display = "none".to_owned();
        assert!(!visible_from(&state, Some(&box_model)));
        state.display = "block".to_owned();
        state.visibility = "hidden".to_owned();
        assert!(!visible_from(&state, Some(&box_model)));
        state.visibility = "visible".to_owned();

        let zero = model(10.0, 20.0, 0.0, 20.0);
        assert!(!visible_from(&state, Some(&zero)));
        assert!(!visible_from(&state, None));
    }

    #[test]
    fn stable_allows_tiny_subpixel_noise_but_rejects_motion() {
        let first = model(10.0, 20.0, 40.0, 20.0);
        let tiny = model(10.1, 20.1, 40.0, 20.0);
        let moved = model(12.0, 20.0, 40.0, 20.0);

        assert!(stable_from(Some(&first), Some(&tiny)));
        assert!(!stable_from(Some(&first), Some(&moved)));
        assert!(!stable_from(Some(&first), None));
    }

    #[test]
    fn failures_report_disabled_obscured_and_not_editable_predicates() {
        let mut state = dom_state();
        state.enabled = false;
        state.disabled = true;
        let hit = CdpActionabilityHitTest {
            point: Some(CdpActionabilityPoint { x: 30.0, y: 30.0 }),
            receives_events: false,
            top_backend_node_id: Some(99),
            top_frame_id: Some("frame-1".to_owned()),
            target_or_descendant_from_point: false,
            top_node_description: Some("div#cover".to_owned()),
            error: None,
        };
        let failures = actionability_failures(
            &state,
            true,
            true,
            Some(&model(0.0, 0.0, 10.0, 10.0)),
            None,
            &hit,
        );
        let pairs = failures
            .iter()
            .map(|failure| (failure.predicate.as_str(), failure.reason.as_str()))
            .collect::<Vec<_>>();

        assert!(pairs.contains(&("enabled", "disabled")));
        assert!(pairs.contains(&("editable", "not_editable")));
        assert!(pairs.contains(&("receives_events", "obscured_or_not_hit")));
        assert!(
            failures
                .iter()
                .any(|failure| failure.detail.contains("div#cover"))
        );
    }
}
