...
mod tests {
    ...
    #[test]
    fn recovery_log_replays_holds_and_removes_when_empty() -> Result<(), Box<dyn Error>> {
        let dir = TempDir::new()?;
        let path = dir.path().join("action_recovery.jsonl");
        let key = named_key("shift");

        append_recovery_event_at(&path, &RecoveryEvent::KeyHeld { key: key.clone() })?;
        append_recovery_event_at(
            &path,
            &RecoveryEvent::ButtonHeld {
                button: MouseButton::Left,
            },
        )?;
        let before = read_ledger_from_log(&path)?;
        assert_eq!(before.keys, vec![key.clone()]);
        assert_eq!(before.buttons, vec![MouseButton::Left]);

        append_recovery_event_at(&path, &RecoveryEvent::KeyReleased { key })?;
        append_recovery_event_at(
            &path,
            &RecoveryEvent::ButtonReleased {
                button: MouseButton::Left,
            },
        )?;
    ...
    #[test]
    fn recovery_log_ignores_trailing_partial_append() -> Result<(), Box<dyn Error>> {
        let dir = TempDir::new()?;
        let path = dir.path().join("action_recovery.jsonl");
        let event = serde_json::to_string(&RecoveryEvent::KeyHeld {
            key: named_key("shift"),
        })?;
        ...
        assert_eq!(ledger.keys, vec![named_key("shift")]);
        assert!(ledger.ignored_trailing_bytes);
        Ok(())
    }
