...
async fn emitter_tracks_held_state_and_release_all_drains() -> Result<(), Box<dyn Error>> {
    ...
    println!("source_of_truth=action_emitter_state edge=happy before={before:?}");
    assert_empty(&before);
    ...
    println!("source_of_truth=action_emitter_state edge=happy held={held:?}");

    handle.execute(Action::ReleaseAll).await?;
    let after = snapshot_handle.snapshot().await?;
    assert_empty(&after);
    println!("source_of_truth=action_emitter_state edge=happy after={after:?} final_value=empty");
...
async fn release_all_on_empty_state_stays_empty() -> Result<(), Box<dyn Error>> {
    ...
    assert_empty(&after);
    println!(
        "source_of_truth=action_emitter_state edge=empty_release after={after:?} final_value=empty"
    );
...
fn emitter_channel_preserves_bounded_queue_capacity() {
    ...
    println!(
        "source_of_truth=action_emitter_queue edge=capacity before=queued:{}",
        emitter.pending_len()
    );
    ...
    println!(
        "source_of_truth=action_emitter_queue edge=capacity after=queued:{} final_value={}",
        emitter.pending_len(),
        error.code()
    );
}
...
async fn cancellation_drains_held_state_before_actor_returns() -> Result<(), Box<dyn Error>> {
    ...
    println!("source_of_truth=action_emitter_state edge=cancel before={before:?}");

    cancel.cancel();
    let after = join.await?;
    assert_empty(&after);
    println!("source_of_truth=action_emitter_state edge=cancel after={after:?} final_value=empty");
    Ok(())
}
...
async fn unmatched_key_up_does_not_create_held_state() -> Result<(), Box<dyn Error>> {
    ...
    assert_empty(&after);
    println!(
        "source_of_truth=action_emitter_state edge=unmatched_key_up after={after:?} final_value=empty"
    );
...
fn randomized_streams_ending_release_all_leave_empty_state() -> Result<(), Box<dyn Error>> {
    ...
    runner.run(&vec(tracked_action_strategy(), 0..=32), |actions| {
        ...
        println!(
            "source_of_truth=action_emitter_state edge=proptest_case_{case_index} final_value=held_keys:{:?} held_buttons:{:?} pad_state:{:?}",
