...
6. **Missing configured-host prerequisites are work, not blockers.** Do not stop at "missing." If the operator could download, install, connect, configure, generate, flash, launch, or inspect it from this computer, the agent must make it happen through Synapse/local host workflows and then inspect the physical SoT. Ask only for narrow approval on hard-to-reverse external actions after every reversible local step is complete.
