...
- **CI test execution.** No GitHub Actions/Makefile/justfile/nextest; the only automated gate is `.githooks/pre-push` running local fmt/clippy over the touched Cargo workspace(s), and the shipping gate ("FSV" = manual Full State Verification) is performed by the agent on the configured Windows host and is never automated. See [17_test_suite.md](17_test_suite.md).
