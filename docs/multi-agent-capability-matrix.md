...
| act_type | action | element target, session-owned Chrome bridge tab, or foreground keyboard target | CDP insertText, normal Chrome bridge `typeActiveElement` for inactive owned `chrome-tab:*` targets, native HWND text message, or UIA ValuePattern for into_element | session-owned Chrome bridge active-element path does not require foreground; remaining no-target keyboard path requires foreground keyboard lease | conditional-pass | #783 #786 #997 | target text before and after plus CF_ACTION_LOG |
...
| browser_evaluate | browser perception/action | active session CDP target or CDP/Chrome bridge target owned by this MCP session | raw CDP Runtime.evaluate in the page main world, or guarded normal Chrome bridge `chrome.debugger.Runtime.evaluate` on a session-owned synthetic `chrome-tab:<id>`; both carry await_promise/return_by_value plus separate URL/title/readyState readback; JS exceptions are surfaced loudly with class/message/location | no foreground lease, no tab activation, no implicit foreground tab fallback, refuses unowned/non-active targets, normal bridge detaches debugger in `finally` and does not refuse solely because the owned target is active/highlighted/focused | foreground-capable-targeted | #1065 #1067 #1063 #1211 #1218 | evaluated JSON value plus Runtime.RemoteObject type/subtype/description readback and CF_ACTION_LOG |
| browser_locate | browser readback | active session CDP target or CDP target owned by this MCP session | raw CDP DOM.querySelectorAll (CSS) or DOM.performSearch (XPath) then DOM.describeNode, mapping matched nodes to Synapse element ids; the popup-safe normal Chrome extension bridge fails closed because it never attaches the debugger | read-only; no foreground lease, no tab activation, no implicit foreground tab fallback, refuses unowned/non-active targets | background-pass | #1111 #1112 #1119 #1110 | resolved element ids, match_count, and url/title readback and CF_ACTION_LOG |
...
| capture_screenshot | perception | explicit `window_hwnd`, session target window, active session CDP/Chrome tab target, current foreground window, or absolute screen region | passive per-window WGC `CreateForWindow` for target/window captures; exact owned CDP/Chrome tab screenshots use raw CDP or guarded normal Chrome bridge `Page.captureScreenshot` with target/window readback; `PrintWindow` is disabled because it re-enters target `WM_PRINT`/`WM_PRINTCLIENT`; legacy GDI screen region only when no target is set | no foreground lease for session targets; target/window regions are client-relative; session-owned Chrome tab capture does not refuse solely because the owned tab is active/highlighted/focused, and reports physical capture backend or explicit disabled/error state | foreground-capable-targeted | #789 #1202 #1218 | image file metadata, bitmap hash, capture_backend/error_code, target/window readback, and CF_ACTION_LOG |
