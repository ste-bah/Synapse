...
`geolocationEmulation`, `localeEmulation`, and `mediaEmulation`. Inactive normal-profile tabs use a strict
...
`geolocationEmulation` / `localeEmulation` / `mediaEmulation` lanes in the already-open
...
`localeEmulation` / `mediaEmulation` metrics lanes plus inactive-tab synthetic MouseEvent drag fallback; DOM attach and
...
`viewportEmulation`, `deviceEmulation`, `geolocationEmulation`,
`localeEmulation`, and `mediaEmulation` are
...
`geolocationEmulation` / `localeEmulation` / `mediaEmulation` lanes require those warnings to be cleared or
